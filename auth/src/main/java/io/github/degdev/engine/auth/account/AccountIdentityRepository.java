...
/** Spring Data repository for {@link AccountIdentity} rows. */
public interface AccountIdentityRepository extends JpaRepository<AccountIdentity, Long> {

  /**
   * Resolves an identity by its natural key — the pair a login presents.
   *
   * @param providerType the authenticating provider
   * @param externalId the provider-scoped identifier
   * @return the matching identity, or empty if none is bound
   */

...
| polish | meta-annotation p2 | BUILD | Y | fix-from-live | browser | rolled @NobilisAdminController to 6 controllers + synced frontend (9 Api paths, auth-interceptor prefix) to /api/admin; back verify + front AOT green; live browser caught dev-server proxy.conf still on old /admin/api prefix → all admin screens broke |
| polish | p2 proxy-fix | FIX | N | fix-from-live | browser | proxy.conf.json key /admin/api → /api/admin (4th path-prefix carrier the rename touched, after controllers/Api-consts/interceptor); ng serve restart → full descriptor→form→save→health loop live green |

...
- **Decisive-first ordering.** This is about the ORDER of tracks *within* a recon that's already been decided on — distinct from KISS above, which decides *whether* to recon at all. When recon already has a strong hypothesis about the cause (especially one the prompt's author already stated in the header), put the cheapest test that would decisively confirm or refute that hypothesis FIRST in the investigation order, not last. The point is to **abort the recon early and spend less time and fewer tokens on it**: if the decisive test confirms, the remaining tracks are often redundant or read differently in its light, and you stop there rather than running them. (Kin to the evidence-first principle — a fact before an opinion; decisive-first applies the same narrowing to track *order*.) (Marked illustration: a bug reproduces on one environment and not another → compare environment state as the first track, not the last.)
